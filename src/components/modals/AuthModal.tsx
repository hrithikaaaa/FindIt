...
import { X, Lock, Mail, User as UserIcon, ArrowRight, Sparkles, ShieldCheck, Loader2 } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authMode === 'login') {
        ...
        await registerWithEmailAuth(email, password, name);
      }
      closeAuthModal();
    } catch (error: any) {
      // Fallback local profile if Firebase email/password user not yet created or offline
      if (error?.code === 'auth/user-not-found' || error?.code === 'auth/wrong-password' || error?.code === 'auth/invalid-credential') {
        ...
        addToast('success', authMode === 'login' ? 'Signed In' : 'Account Created', `Welcome, ${fallbackUser.name}!`);
        closeAuthModal();
      }
    } finally {
      setLoading(false);
        ...
        >
          ...
          {authMode !== 'forgot' && (
            <div className="space-y-3">
              <button
                ...
                disabled={loading}
                className="w-full py-2.5 px-4 rounded-xl border border-slate-300 hover:border-slate-400 bg-white hover:bg-slate-50 text-slate-800 font-semibold text-xs transition-colors flex items-center justify-center gap-2.5 shadow-xs cursor-pointer disabled:opacity-60"
              >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs shadow-md transition-colors flex items-center justify-center gap-1.5 cursor-pointer mt-2 disabled:opacity-60"
            >
              {loading ? (
                <>
